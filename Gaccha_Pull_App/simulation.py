...
def simulate_combo_verbose(
    ...
    weapon_cfg   = weapon_pity_config   or WEAPON_PITY_DEFAULTS
    ...
    pity_weapon, guarantee_weapon = start_weapon_pity, start_weapon_guarantee
    pity_4star_char = 0
    pity_4star_weapon = 0
    ...
    weapon_75_25_encounters = 0
    lost_char_50_50_failure = False
    pity_char_trigger = 0
    pity_weapon_trigger = 0
    ...
    for phase in strategy:
        ...
        while copies_got < copies_needed and remaining > 0:
            if banner == "char":
                ...
                hit_5star = np.random.rand() < banner_probability(pity_weapon, **weapon_cfg)

                if hit_5star:
                    pity_weapon_trigger = pity_weapon
                ...
                elif simulate_4star and (pity_4star_weapon >= 10 or np.random.rand() < 0.06):
                    pity_4star_weapon = 0
                    remaining += 0.4
                    refunded_pulls += 0.4
                    phase_refunds += 0.4
...
def run_simulation_verbose(
    ...
    # Updated: Calculate average pity for char and weapon on success using DataFrame
    avg_pity_char_success = round(df[df.success]["pity_char_trigger"].mean(), 2) if not df[df.success].empty else None
    avg_pity_weapon_success = round(df[df.success]["pity_weapon_trigger"].mean(), 2) if not df[df.success].empty else None
    ...
    # Sum the char_50_50_encounters and weapon_75_25_encounters for successful runs to get accurate totals
    total_50_50s_encountered_in_successes = df[df.success]["char_50_50_encounters"].sum()
    total_25_75s_encountered_in_successes = df[df.success]["weapon_75_25_encounters"].sum()
    ...
    # Updated: Calculate average pity for char and weapon on failure using DataFrame
    avg_pity_char_failure = round(df[~df.success]["final_pity_char"].mean(), 2) if not df[~df.success].empty else None
    avg_pity_weapon_failure = round(df[~df.success]["final_pity_weapon"].mean(), 2) if not df[~df.success].empty else None
    ...
    # Sum the char_50_50_encounters and weapon_75_25_encounters for failed runs to get accurate totals
    total_50_50s_encountered_in_failures = df[~df.success]["char_50_50_encounters"].sum()
    total_25_75s_encountered_in_failures = df[~df.success]["weapon_75_25_encounters"].sum()
    ...
    # Failure state distribution — most common (chars, weapons) combos among failed runs
    fail_df = df[~df.success]
    if not fail_df.empty:
        state_counts = (
            fail_df.groupby(["chars_obtained", "weapons_obtained"])
            .size()
        ...
        top_states = [
            {
                ...
                "weapons": int(row["weapons_obtained"]),
    ...
    # Weapon 75/25 wins: runs where weapon_75_25_wins >= desired_weapons
    success_all_weapon_won = int((df[df.success]["weapon_75_25_wins"] >= desired_weapons).sum()) if desired_weapons > 0 else None
    fail_all_weapon_won    = int((df[~df.success]["weapon_75_25_wins"] >= desired_weapons).sum()) if desired_weapons > 0 else None
    ...
    correlation_stats = {
        ...
        "all_weapon_75s_won_in_successes": success_all_weapon_won,
        "all_weapon_75s_won_in_failures": fail_all_weapon_won,
    ...
    stats_summary = {
        ...
        "avg_pity_weapon": round(df[df.success]["pity_weapon_trigger"].mean(), 2) if not df[df.success].empty else None,
        "successes_char_win_rate": successes_char_win_rate,
        "successes_weapon_win_rate": successes_weapon_win_rate,
        ...
        "failure_weapon_win_rate": failure_weapon_win_rate,
